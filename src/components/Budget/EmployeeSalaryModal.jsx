...
function EmployeeSalaryModal({
  ...
  // Initialize entries for each department with remaining salary
  const [entries, setEntries] = useState(
    activeBudgets.map((b) => ({
      department: b.department,
      amount: "",
      remainingSalary:
        b.departmentExpenses?.employeeSalary?.allocated -
          b.departmentExpenses?.employeeSalary?.spent || 0,
    }))
  );
  ...
  const handleSubmit = (e) => {
    ...
    const validEntries = entries.filter(
      (e) =>
        ...
        Number(e.amount) > 0 &&
        !isNaN(e.amount) &&
        Number(e.amount) <= e.remainingSalary
    );

    if (validEntries.length === 0) {
      alert("Please enter at least one valid salary amount within budget.");
      return;
    }
