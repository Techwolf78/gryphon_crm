...
import tasksData from "./it_task.js";
import {
  doc,
  serverTimestamp,
  onSnapshot,
...
import EditTaskModal from "./ITEditTaskModal";
...
const TaskCard = ({ task, getRoleDisplay, handleDelete, moveTask, onImageDelete, onEditTask, onDuplicateTask, isDraggable = true, user }) => {
  ...
  const roleColors = {
    ...
    Backend: "bg-teal-50 text-teal-700 border-teal-100",
    Tester: "bg-orange-50 text-orange-700 border-orange-100",
    Deployment: "bg-purple-50 text-purple-700 border-purple-100",
  };
...
const ITTaskManager = ({ onBack }) => {
  ...
  const getNextTaskId = async () => {
    const counterRef = doc(db, "counters", "it_task_counter");
    return await runTransaction(db, async (transaction) => {
      ...
      return `ittask${nextId.toString().padStart(4, "0")}`;
    });
  };
  ...
  useEffect(() => {
    ...
    const cachedAssignees = localStorage.getItem("itAssignees");
    if (cachedAssignees) {
      const parsedAssignees = JSON.parse(cachedAssignees);
      setAssignees(parsedAssignees);
      const filterPreference = localStorage.getItem("itFilterPreference") || "user";
    ...
    const fetchUsers = async () => {
      try {
        ...
        const dmUsers = usersSnapshot.docs
          ...
          .filter((user) => {
            const hasIT = user.departments && (user.departments.includes("IT") || user.departments.includes("DM"));
            const hasAdminDept = user.departments && user.departments.includes("Admin");
            return hasIT || hasAdminDept;
          })
          .map((user) => user.displayName || user.name || user.email)
          .filter(Boolean);
        setAssignees(dmUsers);
        localStorage.setItem("itAssignees", JSON.stringify(dmUsers));
        const filterPreference = localStorage.getItem("itFilterPreference") || "user";
        const userName = user?.displayName;
        if (
          filterPreference === "user" &&
  ...
  useEffect(() => {
    ...
    const fetchAllTasks = async () => {
      try {
        const tasksRef = collection(db, "it_tasks");
      ...
      try {
        ...
        if (imageFile) {
          uploadImage(imageFile)
            .then((imageUrl) => {
              updateDoc(doc(db, "it_tasks", taskId), {
                images: [imageUrl],
              });
              setTasks((prev) =>
                prev.map((t) =>
        ...
        const docRef = doc(db, "it_tasks", taskId);
  ...
  const moveTaskImmediate = async (taskId, newStatus) => {
    const taskRef = doc(db, "it_tasks", taskId);
  ...
  const handleSaveTaskDates = async (taskId, taskData) => {
    ...
    try {
      await updateDoc(doc(db, "it_tasks", taskId), taskData);
  ...
  const handleDeleteImmediate = async (id) => {
    await deleteDoc(doc(db, "it_tasks", id));
  };
  ...
  const handleImageDelete = async (taskId, imageIndex) => {
    ...
    try {
      await setDoc(doc(db, "it_tasks", taskId), {
  ...
  const logMarketingActivity = async (action, entityData = {}) => {
    try {
      ...
      await addDoc(collection(db, "it_audit_logs"), auditLog);
    } catch (error) {
