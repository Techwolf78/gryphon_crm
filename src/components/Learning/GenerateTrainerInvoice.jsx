...
import {
  logInvoiceAction,
  AUDIT_ACTIONS,
} from "../../utils/trainerInvoiceAuditLogger";
...
function GenerateTrainerInvoice() {
  ...
  // Trainer summary expansion state
    ...
    return () => {
      try {
        const sessionVisits = parseInt(
          localStorage.getItem("trainer_invoice_session_visits") || "0",
        );
        // If user has visited multiple times in this session, extend cache duration
        const duration =
          sessionVisits > 3 ? EXTENDED_CACHE_DURATION : CACHE_DURATION;
        return duration;
      } catch {
  ...
  const trackUserVisit = () => {
    try {
      ...
      localStorage.setItem(
        "trainer_invoice_session_visits",
  ...
  const fetchTrainers = useCallback(
    async (forceRefresh = false) => {
      ...
      if (
        ...
        cachedData &&
        lastFetchTime &&
        now - lastFetchTime < adaptiveDuration
      ) {
      ...
      try {
                ...
                (batch.batches || []).forEach((b) => {
                  (b.trainers || []).forEach((trainer) => {
                    ...
                    if (domainData.domain === "JD") {
                      ...
                      startDate =
                        trainer.startDate ||
                        domainData.trainingStartDate ||
                        trainer.activeDates?.[0] ||
                        "";
                      endDate =
                        trainer.endDate ||
                        domainData.trainingEndDate ||
                        trainer.activeDates?.slice(-1)[0] ||
                        "";
                    }
                    ...
                    if (
                      ...
                      typeof activeDates[0] === "object" &&
                      activeDates[0]?.toDate
                    ) {
                      activeDates = activeDates.map(
                    ...
                    const trainerObj = {
                      ...
                      collegeName:
                        ...
                        (formData.businessName || "").split("/")[0]?.trim() ||
                        "Unknown College",
                      startDate,
                      endDate,
        ...
        const trainerIds = [
          ...new Set(trainersList.map((t) => t.trainerId).filter((id) => id)),
        ];
          ...
          const trainerPromises = trainerIds.map(async (trainerId) => {
            try {
              ...
              if (trainerSnap.exists()) {
                ...
                const correctName =
                  data.name || data.trainerName || data.displayName;
                if (correctName) {
        ...
        trainersList.forEach((trainer) => {
          if (trainer.trainerId && trainerNamesMap.has(trainer.trainerId)) {
            ...
            if (
              dbName &&
              dbName.length >= 3 &&
              !dbName.includes("excel") &&
              !dbName.includes("traienr") &&
              !dbName.includes("N/A")
            ) {
            ...
            trainer.trainerId &&
            missingTrainers.has(trainer.trainerId)
          ) {
            ...
            if (
              ...
              currentName.includes("excel") ||
              currentName.includes("traienr") ||
              currentName === "N/A"
            ) {
        ...
        trainersList.forEach((trainer) => {
          ...
          paymentCycles.forEach((cycle) => {
            ...
            if (trainer.activeDates && trainer.activeDates.length > 0) {
              ...
              cycleAssignments = trainer.activeDates
                .filter((dateStr) => getPaymentCycle(dateStr) === cycle)
                .sort();
              ...
              hasValidDatesForCycle =
                (trainingStartDay <= cycleEnd &&
                  trainingEndDay >= cycleStart) ||
            ...
            if (!collegePhaseBasedGrouping[groupingKey]) {
              collegePhaseBasedGrouping[groupingKey] = {
                ...
                allColleges: trainer.isMergedTraining
                  ? trainer.mergedColleges.map((c) => c.collegeName || c)
                  : [trainer.collegeName],
            ...
            collegePhaseBasedGrouping[groupingKey].startDate =
              cycleStartDate || trainer.startDate;
            collegePhaseBasedGrouping[groupingKey].endDate =
              cycleEndDate || trainer.endDate;
            ...
            const cycleTrainer = {
              ...
              activeDates:
                trainer.activeDates?.filter(
                  (dateStr) => getPaymentCycle(dateStr) === cycle,
                ) || [],
              dailyHours:
                trainer.dailyHours?.slice(0, cycleAssignments.length) || [],
            };

            collegePhaseBasedGrouping[groupingKey].allBatches.push(
              cycleTrainer,
            );
            ...
            if (
              ...
              new Date(cycleStartDate || trainer.startDate) <
                new Date(
                  collegePhaseBasedGrouping[groupingKey].earliestStartDate,
                )
            ) {
            ...
            if (
              ...
              new Date(cycleEndDate || trainer.endDate) >
            ...
            if (trainer.isMergedTraining) {
              const collegeName = trainer.mergedColleges
                .map((c) => c.collegeName || c)
                .find(
                  (c) =>
                    !collegePhaseBasedGrouping[
                      groupingKey
                    ].allColleges.includes(c),
                );
              if (
                collegeName &&
                !collegePhaseBasedGrouping[groupingKey].allColleges.includes(
                  collegeName,
                )
              ) {
                collegePhaseBasedGrouping[groupingKey].allColleges.push(
                  collegeName,
                );
              }
            }
        ...
        const collegePhaseBasedTrainers = Object.values(
          ...
          const finalTrainer = {
            ...
            totalFood:
              trainer.domain === "JD"
                ? (trainer.allBatches[0]?.food || 0) *
                  (trainer.allBatches[0]?.activeDates?.length ||
                    getTrainingDays(
                      trainer.allBatches[0]?.startDate,
                      trainer.allBatches[0]?.endDate,
                      trainer.excludeDays,
                    ))
                : trainer.allBatches.reduce((sum, batch) => {
                    const days =
                      batch.activeDates?.length ||
                      getTrainingDays(
                        batch.startDate,
                        batch.endDate,
                        trainer.excludeDays,
            ...
            totalLodging:
              trainer.domain === "JD"
                ? (trainer.allBatches[0]?.lodging || 0) *
                  (trainer.allBatches[0]?.activeDates?.length ||
                    getTrainingDays(
                      trainer.allBatches[0]?.startDate,
                      trainer.allBatches[0]?.endDate,
            ...
            totalStudents: trainer.allBatches.reduce(
              (sum, batch) =>
                sum + (batch.batchPerStdCount || batch.stdCount || 0),
              0,
            ),
          };
  ...
  useEffect(() => {
    ...
    const storedCacheVersion = localStorage.getItem(
      "trainer_invoice_cache_version",
    );
    ...
    const adaptiveDuration = getAdaptiveCacheDuration();
    const isCacheValid =
      cachedData && lastFetchTime && now - lastFetchTime < adaptiveDuration;
  ...
  const getCacheStatus = useCallback(() => {
    if (!lastFetchTime)
      return { status: "never", message: "Never loaded", isExpired: false };
  ...
  const filteredGroupedData = useMemo(() => {
    return Object.keys(groupedData).reduce((acc, college) => {
      ...
      Object.keys(groupedData[college]).forEach((phase) => {
        const filteredTrainers = groupedData[college][phase].filter(
          (trainer) => {
            ...
            const matchesDateRange =
              (!startDateFilter ||
                new Date(trainer.earliestStartDate) >=
  ...
  const projectCodes = useMemo(
    () =>
      [...new Set(trainerData.map((item) => item.projectCode))].filter(Boolean),
    [trainerData],
  );
  ...
  const businessNames = useMemo(
    () =>
      [...new Set(trainerData.map((item) => item.businessName))].filter(
        Boolean,
      ),
    [trainerData],
  );

  // Trainer summary for search results - MEMOIZED
  const trainerSummary = useMemo(() => {
    if (!searchTerm.trim()) return null;

    // Use filteredGroupedData instead of trainerData to respect invoice status filter
    const allFilteredTrainers = [];
    Object.values(filteredGroupedData).forEach((collegeData) => {
      Object.values(collegeData).forEach((phaseData) => {
        allFilteredTrainers.push(...phaseData);
      });
    });

    const matchingTrainers = allFilteredTrainers.filter(
      (trainer) =>
    ...
    const uniqueTrainers = [
      ...new Set(matchingTrainers.map((t) => t.trainerId)),
    ];
    if (uniqueTrainers.length !== 1) return null; // Only show summary if exactly one trainer is found

    const trainerId = uniqueTrainers[0];
    const trainerName = matchingTrainers.find(
      (t) => t.trainerId === trainerId,
    )?.trainerName;

    // Calculate total hours and cost from filtered matching entries
    const totalHours = matchingTrainers.reduce(
      (sum, t) => sum + (t.totalCollegeHours || t.assignedHours || 0),
      0,
    );
    ...
    const baseTrainingFees = totalCost;
    const gstAmount = 0; // Default to 0 as per TrainerRow logic
    const taxableAmount = baseTrainingFees + gstAmount;
    const tdsAmount = Math.round(taxableAmount * 0.1); // Default 10% TDS
    const totalConveyance = matchingTrainers.reduce(
      (sum, t) => sum + (t.totalConveyance || 0),
      0,
    );
    const totalFood = matchingTrainers.reduce(
      (sum, t) => sum + (t.totalFood || 0),
      0,
    );
    const totalLodging = matchingTrainers.reduce(
      (sum, t) => sum + (t.totalLodging || 0),
      0,
    );
    const otherExpenses = totalConveyance + totalFood + totalLodging;
    ...
    matchingTrainers.forEach((trainer) => {
      const key = trainer.projectCode;
      const hours = trainer.totalCollegeHours || trainer.assignedHours || 0;
      if (hoursBreakdown[key]) {
        hoursBreakdown[key] += hours;
      } else {
        hoursBreakdown[key] = hours;
      }
    });
    ...
    return {
      ...
      totalCost: Math.round(netPayment), // Use net payment as total cost
      projectCount: [...new Set(matchingTrainers.map((t) => t.projectCode))]
        .length,
      hoursBreakdown,
      // Earnings breakdown
      breakdown: {
        baseTrainingFees: Math.round(baseTrainingFees),
        gstAmount: Math.round(gstAmount),
        taxableAmount: Math.round(taxableAmount),
        tdsAmount: Math.round(tdsAmount),
        otherExpenses: Math.round(otherExpenses),
        totalConveyance: Math.round(totalConveyance),
        totalFood: Math.round(totalFood),
        totalLodging: Math.round(totalLodging),
        netPayment: Math.round(netPayment),
      },
    };
  }, [filteredGroupedData, searchTerm]);
  ...
  const handleDownloadInvoice = useCallback(async (trainer) => {
    ...
    try {
      ...
      if (allInvoices.length === 0 || trainer.invoiceCount > 1) {
        ...
        allInvoices = querySnapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));
      }
      ...
      if (allInvoices.length > 0) {
        if (allInvoices.length > 1) {
          ...
          const invoiceOptions = allInvoices.map(
            (invoice) => `${invoice.billNumber} (${trainer.collegeName})`,
          );
          ...
          if (selectedInvoice) {
            ...
            if (selectedInvoiceData) {
              ...
              setToast({
                type: "error",
                message: "Invalid invoice number selected",
              });
              setPdfStatus((prev) => ({
                ...prev,
                [statusKey]: "error",
        ...
        setToast({
          type: "warning",
          message: "No invoice found for this trainer",
        });
        setPdfStatus((prev) => ({
          ...prev,
          [statusKey]: "not_found",
    ...
    async (trainer) => {
      try {
        ...
        if (!querySnapshot.empty) {
          ...
          setToast({
            type: "success",
            message: `Invoice approved and sent to HR for review`,
          });
        } else {
      ...
      try {
        ...
        if (!querySnapshot.empty) {
          ...
          await logInvoiceAction(
            ...
            {
              deleteReason:
                "Deleted to allow separate invoices per payment cycle",
          ...
          setToast({
            type: "success",
            message: `Invoice deleted successfully. You can now generate separate invoices for each payment cycle.`,
          });
        } else {
          setToast({ type: "error", message: "Invoice not found" });
        }
    ...
    <div className="min-h-screen bg-gray-50">
      {loading ? (
        ...
        <div className="mx-auto bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100">
            ...
            <div className="px-4 py-1 bg-linear-to-r from-blue-50 to-indigo-50 border-b border-blue-100">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    <FiRefreshCw
                      className={`${refreshing ? "animate-spin" : ""} text-xs`}
                    />
          ...
          {trainerSummary && (
            <div className="mx-1 sm:mx-2 mb-1">
              <div className="bg-linear-to-r from-blue-50 via-indigo-50 to-purple-50 border border-blue-200/50 rounded-lg p-1 shadow-md hover:shadow-lg transition-all duration-200">
                <div
                  className="flex items-center justify-between cursor-pointer"
                  onClick={() => setIsSummaryExpanded(!isSummaryExpanded)}
                >
                  <h2 className="text-sm font-semibold text-gray-800">
                    Trainer Summary for "{searchTerm}"
                  </h2>
                  {isSummaryExpanded ? (
                ...
                {isSummaryExpanded && (
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 mt-1">
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <div className="relative">
                          <div className="w-6 h-6 bg-linear-to-br from-blue-500 to-indigo-600 rounded-full flex items-center justify-center shadow-sm">
                      ...
                      <div className="flex flex-col gap-0.5 text-xs text-gray-600">
                        <div className="flex gap-1">
                          <span className="bg-blue-100 text-blue-700 px-1 py-0.5 rounded border text-xs w-fit">
                            ID: {trainerSummary.trainerId}
                          </span>
                          <span className="bg-green-100 text-green-700 px-1 py-0.5 rounded border text-xs w-fit">
                            ₹{trainerSummary.totalHours > 0 ? Math.round(trainerSummary.breakdown.baseTrainingFees / trainerSummary.totalHours).toLocaleString('en-IN') : 0}/hr
                          </span>
                        </div>
                        <span className="bg-purple-100 text-purple-700 px-1 py-0.5 rounded border text-xs w-fit">
                          {trainerSummary.projectCount} Project{trainerSummary.projectCount !== 1 ? 's' : ''}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex justify-end gap-1 lg:gap-2">
                      {/* Hours Breakdown Table - Compact */}
                      <div className="w-72 bg-white/70 border border-gray-200 rounded-md p-1">
                        ...
                        <div className="space-y-0.5 text-xs max-h-20 overflow-y-auto">
                          {Object.entries(trainerSummary.hoursBreakdown).map(
                            ([project, hours]) => (
                              <div key={project} className="flex justify-between">
                                <span
                                  className="text-gray-600 truncate mr-1"
                                  title={project}
                                >
                                  {project}:
                                </span>
                          ...
                          <div className="border-t border-gray-300 pt-0.5 mt-0.5">
                            <div className="flex justify-between font-bold text-blue-600 text-xs">
                              <span>Total:</span>
                              <span>{trainerSummary.totalHours} hrs</span>
                            </div>
                            <div className="flex justify-between text-gray-600 mt-0.5 text-xs">
                              <span>Avg/Project:</span>
                              <span>
                                {trainerSummary.projectCount > 0
                                  ? Math.round(
                                      trainerSummary.totalHours /
                                        trainerSummary.projectCount,
                                    )
                                  : 0}{" "}
                                hrs
                              </span>
                            ...
                            <div className="flex justify-between text-gray-600 text-xs">
                              <span>Projects:</span>
                              <span>{trainerSummary.projectCount}</span>
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="w-72 bg-white/70 border border-gray-200 rounded-md p-1">
                        ...
                        <div className="grid grid-cols-2 gap-0.5 text-xs">
                          <div className="flex justify-between">
                            ...
                            <span className="font-medium">
                              ₹
                              {trainerSummary.breakdown.baseTrainingFees.toLocaleString(
                                "en-IN",
                              )}
                            </span>
                          </div>
                          <div className="flex justify-between">
                            ...
                            <span className="font-medium">
                              ₹
                              {trainerSummary.breakdown.gstAmount.toLocaleString(
                                "en-IN",
                              )}
                            </span>
                          </div>
                          <div className="flex justify-between">
                            ...
                            <span className="font-medium text-red-600">
                              -₹
                              {trainerSummary.breakdown.tdsAmount.toLocaleString(
                                "en-IN",
                              )}
                            </span>
                          </div>
                          {trainerSummary.breakdown.totalConveyance > 0 && (
                            <div className="flex justify-between">
                              <span className="text-gray-600">Conveyance:</span>
                              <span className="font-medium">
                                ₹
                                {trainerSummary.breakdown.totalConveyance.toLocaleString(
                                  "en-IN",
                                )}
                              </span>
                            </div>
                          )}
                          {trainerSummary.breakdown.totalFood > 0 && (
                            <div className="flex justify-between">
                              <span className="text-gray-600">Food:</span>
                              <span className="font-medium">
                                ₹
                                {trainerSummary.breakdown.totalFood.toLocaleString(
                                  "en-IN",
                                )}
                              </span>
                            </div>
                          )}
                          {trainerSummary.breakdown.totalLodging > 0 && (
                            <div className="flex justify-between">
                              <span className="text-gray-600">Lodging:</span>
                              <span className="font-medium">
                                ₹
                                {trainerSummary.breakdown.totalLodging.toLocaleString(
                                  "en-IN",
                                )}
                              </span>
                            </div>
                          ...
                          <div className="flex justify-between">
                            ...
                            <span className="font-medium">
                              ₹
                              {trainerSummary.breakdown.taxableAmount.toLocaleString(
                                "en-IN",
                              )}
                            </span>
                          </div>
                          <div className="col-span-2 border-t border-gray-300 pt-0.5 mt-0.5">
                            <div className="flex justify-between font-bold text-green-600">
                              <span>Net Payment:</span>
                              <span>
                                ₹
                                {trainerSummary.breakdown.netPayment.toLocaleString(
                                  "en-IN",
                                )}
                              </span>
                            </div>
      ...
      {toast && (
        <div className="fixed bottom-4 right-4 z-50">
          <div
            className={`px-4 py-3 rounded-lg shadow-lg border text-sm font-medium transition-all duration-300 ${
              toast.type === "success"
