...
const RaiseInvoiceModal = ({ isOpen, invoice, onClose, onSubmit, isEdit, paymentType, existingInvoices }) => {
  ...
  useEffect(() => {
    if (invoice) {
      ...
      if (isEdit) {
        setFormData({
          totalContractValue: invoice.netPayableAmount || invoice.totalCost || '',
  ...
  const handleFormChange = (e) => {
    ...
    if (name === "installment" && invoice && invoice.paymentDetails) {
      const selectedInstallment = invoice.paymentDetails.find(p => p.name === value);
      setFormData(prev => ({ 
        ...prev, 
        [name]: value,
        amountRaised: selectedInstallment ? selectedInstallment.totalAmount : prev.amountRaised
      }));
    } else {
  ...
  const getPaymentTypeName = (type) => {
    switch(type) {
      case "AT": return "Advanced Training";
      case "ATP": return "Advanced Training Placement";
      case "ATTP": return "Advanced Training Training Placement";
      case "ATTT": return "Advanced Training Training Training";
      case "EMI": return "Equated Monthly Installment";
      default: return type;
    }
  };
      ...
      <div className="p-4 border w-96 shadow-lg rounded-md bg-white">
        <div className="text-center">
          <h3 className="text-lg font-medium text-gray-900">
            {isEdit ? 'Edit Invoice' : `Raise ${getPaymentTypeName(paymentType)} Invoice`}
          </h3>
              ...
              >
                ...
                {availableInstallments.map((payment, index) => (
                  <option key={index} value={payment.name}>
                    {payment.name} ({payment.percentage}%) - ₹{payment.totalAmount}
                  </option>
                ))}
              </select>
            </div>
